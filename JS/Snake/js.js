...
const snake = {
   ...
   */
  getNextStepHeadPoint(rowsCount, collsCount) {
    console.log(`rowsCount - ${this.rowsCount} , collCount - ${this.collsCount}`);
    ...
    switch (this.direction) {
      case 'up':
        if (firstPoint.y <= 0){
          return {x: firstPoint.x, y: this.collsCount};
        }
        return {x: firstPoint.x, y: firstPoint.y - 1};
      case 'right':
        if (firstPoint.x >= this.collsCount){
          return {x: 0, y: firstPoint.y};
        }
        return {x: firstPoint.x + 1, y: firstPoint.y};
      case 'down':
        if (firstPoint.y >= this.rowsCount){
          return {x: firstPoint.x, y: 0};
        }
        return {x: firstPoint.x, y: firstPoint.y + 1};
      case 'left':
        if (firstPoint.x <= 0){
          return {x: this.collsCount, y: firstPoint.y};
        }
        return {x: firstPoint.x - 1, y: firstPoint.y};
    }
  },
...
const game = {
  ...
  giveSnakeMap(row, col){
    //Object.defineProperty(obj, prop, descriptor)
    //Object.defineProperty(this.snake, 'rowsCount', {value: row});
    //Object.defineProperty(this.snake, 'collsCount', {value: col});
    this.snake.rowsCount = row;
    this.snake.collsCount = col;
    //this.snake.colls: col;

  },

  scoreCounterCreate(){
    const wrap = document.createElement('div');
    wrap.setAttribute('id','counter');
    document.getElementById('game-wrap').insertBefore(wrap, document.getElementById('game'));
  },
  ...
  reset() {
    ...
    this.giveSnakeMap(this.config.settings.rowsCount, this.config.settings.colsCount);
  ...
  render() {
    ...
    counter.innerText = `Счет : ${this.snake.getBody().length - 1}`
